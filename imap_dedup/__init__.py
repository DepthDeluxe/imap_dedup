...
def create_message_record(folder: str, message: Dict) -> MessageRecord:
  ...
  return MessageRecord(folder=folder, seq=seq, subject=_optional_utf8(envelope.subject), message_id=_optional_utf8(envelope.message_id), date_=date_, size=size)

def create_database_tables(cursor: sqlite3.Cursor) -> None:
  cursor.execute('CREATE TABLE IF NOT EXISTS messages (folder VARCHAR(128), seq INT, subject TEXT, message_id TEXT, date DATETIME, size INT, PRIMARY KEY (folder, seq))')
  cursor.execute('CREATE TABLE IF NOT EXISTS actions (folder VARCHAR(128), seq INT, action TEXT, completed_at DATETIME, PRIMARY KEY (folder, seq))')
...
def insert_message_record(cursor: sqlite3.Cursor, message_record: MessageRecord) -> None:
  global detected_duplicates
  try:
    cursor.execute('INSERT INTO messages VALUES (?, ?, ?, ?, ?, ?)', [message_record.folder, message_record.seq, message_record.subject, message_record.message_id, message_record.date_, message_record.size])
  except sqlite3.IntegrityError as e:
    if _is_unique_constraint_violation(e):
      if not detected_duplicates:
        logging.warning('Detected duplicates (example: folder=%s, seq=%d)', message_record.folder, message_record.seq)
        detected_duplicates = True
    else:
      raise e

def message_record_exists(cursor: sqlite3.Cursor, folder: str, seq: int) -> bool:
  return len(cursor.execute('SELECT folder, seq FROM messages WHERE folder=? and seq=?', [folder, seq]).fetchall()) > 0

def insert_action_record(cursor: sqlite3.Cursor, action_record: ActionRecord) -> None:
  try:
    cursor.execute('INSERT INTO actions (folder, seq, action) VALUES (?, ?, ?)', [action_record.folder, action_record.seq, action_record.action.name])
  except sqlite3.IntegrityError as e:
...
def find_pending_action_records(cursor: sqlite3.Cursor) -> List[ActionRecord]:
  ...
  '''
  return [ActionRecord(r[0], r[1], r[2], None) for r in cursor.execute(query).fetchall()]
...
def find_action_by_message(cursor: sqlite3.Cursor, message: MessageRecord):
  ...
  '''
  return [ActionRecord(folder=message.folder, seq=message.seq, action=r[0], completed_at=None) for r in cursor.execute(query, [message.folder, message.seq]).fetchall()]
...
def find_message_records_with_id(cursor: sqlite3.Cursor, message_id: str) -> List[MessageRecord]:
  query = '''
  SELECT
    folder,
    seq,
  ...
  WHERE message_id=?
  '''
  return [MessageRecord(r[0], r[1], r[2], r[3], r[4], r[5]) for r in cursor.execute(query, [message_id]).fetchall()]
...
@retry(retry_on_exception=lambda e: isinstance(e, BadFetchError), stop_max_attempt_number=10, wait_exponential_max=1000, wait_exponential_multiplier=50)
def load_messages_fetched_from_imap_with_retry(connection, server, message_seq_chunk, folder):
  """
  :connection: The connection to the SQLite database
  :server: IMAP server connection
  """
  seqs_to_fetch = [s for s in message_seq_chunk if not message_record_exists(connection.cursor(), folder, s)]

  messages = server.fetch(seqs_to_fetch, [MESSAGE_ENVELOPE, MESSAGE_DATE, MESSAGE_SIZE])
  for seq in messages.keys():
    try:
      record = create_message_record(folder, messages[seq])
    except BadFetchError as e:
      logging.warning('Detected bad fetch for folder %s, messages starting at %d', folder, seq)
      connection.rollback()
      raise e
...
@click.group()
@click.option('--hostname', default='imap.mail.me.com', type=str, help='Hostname of IMAP server')
@click.option('--username', required=True, type=str, help='Username for IMAP authentication')
@click.option('--password', required=True, type=str, help='Password for IMAP authentication')
...
def pull(ctx):
  ...
  for folder in tqdm([f[2] for f in ctx.server.list_folders()], desc='Folders'):
    ...
    for message_seq_chunk in tqdm(chunk(ctx.server.search(), BATCH_FETCH_LIMIT), desc='Messages', colour='green', leave=False, unit_scale=BATCH_FETCH_LIMIT):
      load_messages_fetched_from_imap_with_retry(ctx.connection, ctx.server, message_seq_chunk, folder)
...
def find_duplicates(ctx, all_mail: str):
  ...
  for index, id_ in enumerate(tqdm(find_duplicate_messages(cursor))):
    for duplicate in [d for d in find_message_records_with_id(cursor, id_) if d.folder == all_mail]:
      insert_action_record(cursor, ActionRecord(folder=duplicate.folder, seq=duplicate.seq, action=Action.DELETE, completed_at=None))
      num_deletions += 1
  ...
  logging.info('Found %d duplicate messages, will perform %d deletions saving %d bytes of space', index, num_deletions, space_saved)
  print(f'Found {index} duplicate messages, will perform {num_deletions} deletions saving {space_saved} bytes of space')
...
def print_duplicates(ctx):
  ...
  for id_ in tqdm(find_duplicate_messages(cursor)):
    ...
    for message in messages:
      ...
      actions.append(f'{message.folder} : {action[0].action if len(action) > 0 else "KEEP"}')
...
def deduplicate(ctx):
  ...
  for folder, actions in tqdm(folder_to_actions.items(), desc='Folders'):
    ...
    for chunk_ in tqdm(chunks, desc='Messages', colour='red', unit_scale=BATCH_FETCH_LIMIT):
